...
All routes are namespaced under `/api/v1`. The `/cable` endpoint is the ActionCable WebSocket mount.

| Method | Path | Controller | Purpose |
|--------|------|------------|---------|
| GET | `/up` | `rails/health#show` | Health check |
| **Threats** | | | |
| GET | `/api/v1/threats` | `ThreatsController#index` | List threats — filterable by status, severity, date, search; paginated |
| GET | `/api/v1/threats/:id` | `ThreatsController#show` | Single threat with full detail and actions |
| PATCH | `/api/v1/threats/:id` | `ThreatsController#update` | Update threat status |
| POST | `/api/v1/threats/:threat_id/actions` | `ThreatActionsController#create` | Record a manual or automated action on a threat |
| **Review Queue** | | | |
| GET | `/api/v1/review_queue` | `ReviewQueueController#index` | List FP review items, filterable by `review_status` |
| PATCH | `/api/v1/review_queue/:id` | `ReviewQueueController#update` | Confirm or reject a false-positive review |
| **Hunt Cycles** | | | |
| GET | `/api/v1/hunts` | `HuntsController#index` | List hunt cycles (paginated) |
| GET | `/api/v1/hunts/:id` | `HuntsController#show` | Hunt detail with associated threats |
| POST | `/api/v1/hunts` | `HuntsController#create` | Manually trigger a new hunt |
| **Chat** | | | |
| GET | `/api/v1/chat/history` | `ChatController#history` | Retrieve COMMANDER conversation history |
| POST | `/api/v1/chat` | `ChatController#create` | Send a message to the COMMANDER agent |
| POST | `/api/v1/chat/create_threat` | `ChatController#create_threat` | Promote a chat message into a Threat record |
| **Dashboard** | | | |
| GET | `/api/v1/dashboard/stats` | `DashboardController#stats` | Aggregate counts and metrics |
| GET | `/api/v1/dashboard/activity_feed` | `DashboardController#activity_feed` | Recent activity feed |
| **Exception Patterns** | | | |
| GET | `/api/v1/exception_patterns` | `ExceptionPatternsController#index` | List all exception patterns |
| POST | `/api/v1/exception_patterns` | `ExceptionPatternsController#create` | Create a new exception pattern |
| PATCH | `/api/v1/exception_patterns/:id` | `ExceptionPatternsController#update` | Update (e.g. toggle active) an exception pattern |
| **Slack** | | | |
| POST | `/api/v1/slack/interactions` | `SlackController#interactions` | Receive Slack interactive-component callbacks |
